...
const ApartmentDetails = () => {
  ...
  const { sender, getSenderAmount, contract, address } = useStateContext();

  const [isLoading, setIsLoading] = useState(false);
  const [amount, setAmount] = useState('');
  const [amountappartment, setamountappartment] = useState([]);

  const remainingDays = daysLeft(state.deadline);

  const fetchamountappartment = async () => {
    const data = await getSenderAmount(state.pId);

    setamountappartment(data);
  }

  useEffect(() => {
    if(contract) fetchamountappartment();
  }, [contract, address])

  const handleSender = async () => {
    setIsLoading(true);

    await sender(state.pId, amount); 
    ...
    <div>
      ...
      <div className="w-full flex md:flex-row flex-col mt-10 gap-[30px]">
        <div className="flex-1 flex-col">
          <img src={state.image} alt="apartment" className="w-full h-[410px] object-cover rounded-xl"/>
          <div className="relative w-full h-[5px] bg-[#3a3a43] mt-2">
            <div className="absolute h-full bg-[#4acd8d]" style={{ width: `${calculateBarPercentage(state.target, state.amountappartment)}%`, maxWidth: '100%'}}>
            </div>
          </div>
        </div>
